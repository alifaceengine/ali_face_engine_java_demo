...
public class FR_recognizeFeature_Demo {
    private static int RunMode = Mode.TERMINAL;
    //private static int RunMode = Mode.CLOUD;
    private static String GROUP_NAME = "SMALL_2000";
    ...
    public static void main(String[] args) {
        ...
        recognizePictures(faceRegister, faceRecognize, sGroup.id);
    ...
    private static void recognizePictures(FaceRegister faceRegister, FaceRecognize faceRecognize, String groupId) {
        FaceDetect faceDetect = FaceDetect.createInstance(RunMode);
        if (faceDetect == null) {
            System.out.println("FaceDetect.createInstance error");
            return;
        ...
        for (int i = 0; i < TEST_PERSONS.length; i++) {
            ...
            String feature = faceRegister.extractFeature(image, faces[0], ModelType.MODEL_3K);
            RecognizeResult recognizeResult = faceRecognize.recognizeFeature(feature);
            if (recognizeResult == null) {
                System.out.println("recognizeFeature error");
            } else {
                if (recognizeResult.personName.equals(personName)) {
                    System.out.println("recognizePicture OK, " + personName + " vs " + recognizeResult);
                } else {
                    System.out.println("recognizePicture Fail, " + personName + " vs " + recognizeResult);
                }
            }
        }
